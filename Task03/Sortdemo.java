...
public class Sortdemo {
	...
	public static void main(String[] args) {
		...
		int runtimeLength =  200;
		int intstr = 0; //0 for int, 1 for string arrays
		...
		try {
			 ...
			 */	
			for(int i=2; i<runtimeLength+1; i++){			
				out.write(i + " | "+ run(i)[0][intstr] + " | " + run(i)[1][intstr] + " | " + run(i)[2][intstr]);
				out.newLine();
			}
			out.close();
		} catch (IOException e) {}
	...
	public static double[][] run(int n)
	{
		 ...
		 */
		Integer[] intArray, intArray2, intArray3;
		 ...
		 * one for binary insertion sort, one for mergesort
		 */
		for(int i=0; i<intArray.length; i++){
